...
import org.example.aijavacreate.ai.AiCodeGenTypeRoutingService;
...
public class AppController {
    ...
    public BaseResponse<Page<AppVO>> listMyAppVOByPage(@RequestBody AppQueryRequest appQueryRequest, HttpServletRequest request) {
        ...
        // 限制每页最多 20 个
        long pageSize = appQueryRequest.getPageSize();// 每页大小
        ThrowUtils.throwIf(pageSize > 20, ErrorCode.PARAMS_ERROR, "每页最多查询 20 个应用");
        long pageNum = appQueryRequest.getPageNum();// 页码
        // 只查询当前用户的应用
        appQueryRequest.setUserId(loginUser.getId());// 设置查询用户id
    ...
    @Cacheable(
            value = "good_app_page",
            key = "T(org.example.aijavacreate.utils.CacheKeyUtils).generateKey(#appQueryRequest)",
